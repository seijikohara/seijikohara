...
describe("computeStreaks", () => {
  ...
  it("counts a single active day", () => {
    expect(computeStreaks([day("2026-07-22", 3)])).toEqual({ current: 1, longest: 1 });
  });
  ...
  it("breaks runs on zero days", () => {
    ...
    expect(computeStreaks(days)).toEqual({ current: 2, longest: 2 });
  });
  ...
  it("ends the current streak when the last two days are zero", () => {
    ...
    expect(computeStreaks(days)).toEqual({ current: 0, longest: 1 });
  });
  ...
  it("spans year boundaries", () => {
    ...
    expect(computeStreaks(days)).toEqual({ current: 4, longest: 4 });
  });
  ...
  it("tracks a longest streak that is not the current one", () => {
    ...
    expect(computeStreaks(days)).toEqual({ current: 1, longest: 3 });
  });
